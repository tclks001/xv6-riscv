...
int fork(void)
{
  ...
  np->sz = p->sz;
  np->tracemask = p->tracemask; // copy trace mask from parent to child
...
// get load average * 100
uint64 get_load_avg(void)
{
  return load_avg;
}

// process whose state is not UNUSED
int nproc(void)
{
  int i, n = 0;
  for (i = 0; i < NPROC; i++)
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        last_timer_ticks = timer_ticks / 10;
        timer_ticks = ticks / 10;
        int elapsed_ticks = timer_ticks - last_timer_ticks; // 10 ticks per second
        while (elapsed_ticks > 0)
        {
          load_avg = (load_avg * 59 + runnable_processes * 100) / 60;
          elapsed_ticks--;
        }
      }
      release(&p->lock);
