...
xv6 应对异常的方式非常无聊：如果异常出现在用户空间，内核就会杀死出错的进程。如果异常出现在内核，内核就会 panic。现实的操作系统往往应对得更加有趣得多。

举个例子，许多内核使用页错误来实现“写时拷贝”（copy-on-write，COW）fork。为了解释什么是写时拷贝 fork，回想一下第三章讲的 xv6 的 fork。`fork`使得子进程有和父进程相同的内存，通过调用`uvmcopy`（kernel/vm.c 第 309 行）给子进程分配物理内存，并且把父进程的内存拷贝进去。如果子进程和父进程能够分享父进程的物理内存，那就会更加高效。然而，直接这么实现会出错，因为这会导致父进程和子进程在写入共享的堆栈时，互相干扰对方的执行。

通过写时拷贝 fork，父子进程可以安全地分享物理内存，这是由页错误驱动的。当 CPU 不能把一个虚拟地址转译为物理地址时，CPU 就会产生一个页错误异常（page-fault exception）。RISC-V 有三种不同的页错误：加载页错误（一个加载指令不能转译它的虚拟地址），存储页错误（一个存储指令不能转译它的虚拟地址），以及指令页错误（指令的地址不能转译）。`scause`寄存器的值表示页错误的类型，`stval`寄存器保存了不能被转译的地址。

COW fork 的基本方案是，父进程和子进程起初分享所有的物理页，但都映射为只读。因此，当子进程或父进程执行存储指令时，RISC-V CPU 就会发出一个页错误异常。于是它在子进程的地址空间中映射一个读写拷贝，在父进程的地址空间映射另一个读写拷贝。在更新页表后，内核再从产生错误的指令处开始，恢复出错的进程。因为内核已经更新了相关的 PTE 允许写入，所以出错的指令现在再执行就不会出错了。

这种 COW 方案对于`fork`效果很好，因为子进程往往在`fork`后立即调用了`exec`，用一个新的地址空间覆盖了原地址空间。在这种常见情况下，子进程只会出现一些页错误，内核避免了完全拷贝。此外，COW fork 是透明的：应用不需要做出任何修改就能从中获益。

页表和页错误的结合开启了很广阔的有趣的可能性，远不止 COW fork。另一种广泛使用的特性称为“懒分配”（lazy allocation），包含两个部分。其一，当应用调用`sbrk`时，内核会增长地址空间，但把新地址在页表里标记为无效。其二，当那些新的地址中的某个出现页错误时，内核再分配物理地址，并映射到页表。因为应用经常索要超出它们需要的内存，所以懒分配的好处是：内核只在应用确实用了的时候再分配内存。和 COW fork 一样，内核实现这种特性对于应用来说也是透明的。

还有另一种广泛使用的基于页错误的特性是“从磁盘分页”（paging from disk）。如果应用需要的内存超出了可用的物理 RAM，内核可以赶出一些页：把它们写到存储设备上，例如磁盘，然后把它们的 PTE 标为无效。如果一个应用读写了某个赶出去的页，CPU 就会出现页错误。之后内核检查出错的地址。如果地址属于一个位于磁盘上的页，内核就会分配一个物理内存，把这个页从磁盘读取到这块内存，然后更新 PTE 为可用，并指向这块内存，然后恢复应用。为了给这个页留出空间，内核必须赶走另一个页。这种特性不需要修改应用，如果应用有引用局部性（例如，它在任何时间都只使用了它的内存的一个子集），那么这种特性会效果很好。

其他的结合页表和页错误异常的特性包括自动扩展栈，以及内存映射文件。

## 4.7 现实世界

如果内核的内存映射到了每个进程各自的用户页表中（带有合适的 PTE 权限标识），那么就不再需要特殊的跳板页了。这样的话，在从用户空间陷入内核时，也不必切换页表。反过来，这会允许在内核中系统调用的实现利用正在映射的当前进程的用户内存，允许内核代码直接引用用户指针。许多操作系统使用这些方法增加效率。xv6 避免了这些方法，目的是减少在内核中由于无意地使用用户指针，而造成的安全错误的机会，也减少了为了确保用户和内核虚拟地址不重叠所需的复杂性。
